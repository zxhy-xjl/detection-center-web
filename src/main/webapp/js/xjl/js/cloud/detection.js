...
function testTerminalInfo(){
	...
	addItemStatus("终端信息::" + msg,json.state);
	startNextCheckItem();
}
//身份证读卡器
...
function testThermoPrinter(){
	addItemStatus("小票打印机::<span id='ThermoPrinter'>正在打印</span>","waiting");
...
function testHSICatch(){
	addItemStatus("高拍仪拍照::<span id='HSICatch'>正在拍摄</span>","waiting");
...
function testA4PrinterPrintUrl(){
	//打印远端url
	addItemStatus("A4打印网络文件::<span id='A4PrinterPrintUrl'>正在打印网络文件</span>","waiting");
...
function testCameraOpen(){
	addItemStatus("摄像头拍照::<span id='CameraOpen'>正在打开摄像头</span>","waiting");
...
function testCameraVideo(){
	addItemStatus("摄像头录像::<span id='CameraVideo'>正在打开摄像头</span>","waiting");
...
function testBarCode(){
	if (typeof RCU.BarCode == "undefined"){
		addItemStatus("扫码::该接口未实现","error");
		startNextCheckItem();
		return;
	}
	addItemStatus("扫码::<span id='BarCode'>正在扫码</span>","waiting");
...
//检测条目
/**
 * 如果还有没有检查完的，其他按钮是不能用的。
 * 1.检查有没有正在做检查，如果正在做检查，直接退出，不做任何执行
 * 2.开始检查，首先清除原有内容
 * 3.开始执行任务
 * @param code 检查代码
 * @param title 检查标题
 * @param items 要检查项目的集合
 * @returns
 */
function checkItem(code,title,items){
	if (itemNow < itemCount){
		return;
	}
	cleanGroup();
	itemCode = code;
	...
	itemNow = 0;
	itemSuccess = 0;
	$("#detectionContentTitle").text("正在检测 " + itemTitle + " 共"+itemCount+"项");
	startNextCheckItem();
}
function startNextCheckItem(){
	showProgressbar();
	if (itemNow<itemCount){
		itemNow++;
		$("#detectionContentTitle").text("正在检测 " + itemTitle + " 共"+itemCount+"项,正在检测第"+itemNow+"项");
		eval(itemArray[itemNow-1]+"()");
	} else {
		if (itemSuccess == itemCount){
			$("#detectionContentTitle").html('检测完成 共'+itemCount+'项，成功'+itemSuccess+'项 失败'+(itemCount-itemSuccess)+'项 <a href="#" id="deployTerminal" onclick="deployTerminalAs()"></a>');
			$("#deployTerminal").text("点击发布该为设备为" + itemTitle + "类型");
		} else {
			$("#detectionContentTitle").html('检测完成 共'+itemCount+'项，成功'+itemSuccess+'项 失败'+(itemCount-itemSuccess)+'项');

		}
	}
}
//添加一个实例item，state为1代表成功，为0代表未知，为-1代表失败
function addItemStatus(item,state){
	if (state=="ok"){
		itemSuccess++;
