...
function testThermoPrinter(){
	addItem("小票打印机::<span id='ThermoPrinter'>正在打印</span>","");
...
function ThermoPrinterCallback(code,message){
	setTimeout(function(){
		...
		resetItemStatus("ThermoPrinter",code == "0",errorMessage);
	},2000*(itemNow));
...
function testA4PrinterPrintUrl(){
	//打印远端url
	addItem("A4打印网络文件::<span id='A4PrinterPrintUrl'>正在打印网络文件</span>","");
...
function testA4PrinterPrintUrlCallback(code, message){
	setTimeout(function(){
		resetItemStatus("A4PrinterPrintUrl",code == 0,code + "(" + message + ")");
	},2000*(itemNow));
}
